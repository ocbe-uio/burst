---
title: "BURST Statistical Per Protocol Report"
...
```
# Introduction
This is the report for the per protocol analysis of the BURST trial where patient 3-009 has been taken out (wrong inclusion). The data are based on an export from the Viedoc electronic data capture at `r export_date` system time stamped "`r params$viedoc_export`". 
...
## Post-hoc outcome: Patient Specific Functional Scale (PSFS) total score  
### Plot
```{r, fig.fullwidth = TRUE, fig.width = 10, fig.height = 7, warning=FALSE}
f_anplot1(adprom, "psfs")
...
res <- f_antbl_overall(adprom, "psfs")
res$tbl
...
By treatment and overall
```{r}
res$by_trt %>%     select(-s.value, -term) %>% 
  knitr::kable(col.names = c("Treatment", "Estimate", "Std. Err.", "p-value", "Lower CL", "Upper CL"), digits = 3)

res$overall %>%    select(-s.value) %>% 
  knitr::kable(col.names = c("Estimate", "Std. Err.", "p-value", "Lower CL", "Upper CL"), digits = 3)
```
